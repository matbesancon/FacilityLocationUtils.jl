...
function parse_facility_location_cfl(filepath)
    ...
    depot_distance_matrix = [
        sqrt(
            (depot_x_coord[i] - depot_x_coord[j])^2 +
            (depot_y_coord[i] - depot_y_coord[j])^2,
        ) for i = 1:ndepots, j = 1:ndepots
    ]
    ...
    depot_customer_distance_matrix = [
        sqrt(
            (depot_x_coord[i] - customer_x_coord[j])^2 +
            (depot_y_coord[i] - customer_y_coord[j])^2,
        ) for i = 1:ndepots, j = 1:ncustomers
    ]
    ...
    if ndepots != ncustomers
        @assert norm(
            depot_customer_distance_matrix_file - depot_customer_distance_matrix,
            Inf,
        ) ≤ 1e-2
    else
        @assert norm(
